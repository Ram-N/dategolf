    ...
    <div class="flex-1 overflow-y-auto px-4 pb-4">
      ...
      <ul v-else class="space-y-2">
        <li
          ...
          :key="event.id"
          class="bg-gray-900 rounded-xl px-4 py-3 cursor-pointer hover:bg-gray-800 transition-colors border border-transparent hover:border-gray-700 active:scale-98"
        ...
        >
          ...
          <div class="text-xs text-gray-500 mt-0.5">
            {{ event.categories.join(' · ') }} · {{ event.region }}
          </div>
        </li>
      </ul>

      <p v-if="results.length > 50" class="text-center text-gray-600 text-xs mt-3">
        Showing top 50 — narrow your search to see more
      </p>
    </div>
  </div>
</template>
